/*
sp_ShortCalls_MgrReview_Submit(01a).sql
...
IF EXISTS (
  SELECT * 
   ...
   WHERE SPECIFIC_SCHEMA = N'EC'
     AND SPECIFIC_NAME = N'sp_ShortCalls_MgrReview_Submit' 
)
   DROP PROCEDURE [EC].[sp_ShortCalls_MgrReview_Submit]
GO
...
-- go
CREATE PROCEDURE [EC].[sp_ShortCalls_MgrReview_Submit] (
  @strUserLanId NVARCHAR(70),
  @intLogId BIGINT,
  @dtConfirmed datetime,
  @strMgrNotes nvarchar(3000),
  @tableSCMgrReview SCMgrReviewTableType READONLY,
...
-------------------------------------------------------------------------------------
   set @storedProcedureName = 'sp_ShortCalls_MgrReview_Submit'
-------------------------------------------------------------------------------------
-- Revision History:
...
SET @strUserID = EC.fn_nvcGetEmpIdFromLanID(@strUserLanId,@dtmDate)
  ...
  SET
  [MgrAgreed] = T.[MgrAgreed],
  [MgrComments] = T.[MgrComments]
  FROM [EC].[ShortCalls_Evaluations]SCL JOIN  @tableSCMgrReview T 
  ON SCL.[VerintCallID] = T.[VerintCallID]
  WHERE SCL.[CoachingID] = @intLogId;

 UPDATE [EC].[Coaching_Log]
SET 
  StatusID = 1,
  Review_MgrID =  @strUserID,
  MgrReviewAutoDate =  @dtmDate ,
  MgrReviewManualDate = @dtConfirmed,
  MgrNotes =  @strMgrNotes,
  ReassignCount = 0
