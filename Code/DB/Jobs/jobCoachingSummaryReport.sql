...
DECLARE @jobName nvarchar(100) = N'CoachingSummaryReport'
DECLARE @configfileName nvarchar(100) = CONCAT(@environment + N'_', N'CoachingSummaryReport.dtsConfig')
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@jobName, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Generate monthly Coaching summary reports.', 
...
DECLARE @packagePath nvarchar(1000) = 
			(SELECT CASE @environment WHEN 'PROD' THEN N'\\UVAAPADSQL50CCO\SSIS\Coaching\Packages\'
									  WHEN 'TEST' THEN N'\\UVAADADSQL52CCO\SSIS\Coaching\Packages\CoachingSummaryReport\' 
									  ELSE N'\\UVAADADSQL50CCO\SSIS\Coaching\Packages\CoachingSummaryReport\' END) -- Assume dev
...
-------------------------------------------------------------------
-- Object 1:  Step [Generate Coaching summary reports]
-------------------------------------------------------------------
DECLARE @packageCommand nvarchar(4000) = N'/FILE "' + @packagePath + N'CoachingSummaryReport.dtsx" /CONFIGFILE "' + @configfilePath + @configfileName +'"  /CHECKPOINTING OFF  /REPORTING E'
DECLARE @stepName nvarchar(100) 
SET @jobId = @mainJobId
SET @stepId = @stepID + 1
SET @stepName =  N'Generate reports'
