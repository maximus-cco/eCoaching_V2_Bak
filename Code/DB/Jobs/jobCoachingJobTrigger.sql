...
-------------------------------------------------------------------
-- Creates the CoachingJobTrigger job for the desired environment
...
DECLARE @jobName nvarchar(100) = N'CoachingJobTrigger'
DECLARE @configfileName nvarchar(100) = CONCAT(@environment + N'_', N'JobTrigger_Coaching.dtsConfig')
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@jobName, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Job to trigger another job when file exists.', 
...
DECLARE @packagePath nvarchar(1000) = 
			(SELECT CASE @environment WHEN 'PROD' THEN N'\\UVAAPADSQL50CCO\SSIS\Coaching\Packages\'
									  WHEN 'TEST' THEN N'\\UVAADADSQL52CCO\SSIS\Coaching\Packages\JobTrigger_Coaching\' 
									  WHEN 'UAT' THEN N'\\UVAADADSQL52CCO\SSIS\Coaching\Packages\JobTrigger_Coaching\' 
									  ELSE N'\\UVAADADSQL50CCO\SSIS\Coaching\Packages\JobTrigger_Coaching\' END) -- Assume dev
...
DECLARE @configfilePath nvarchar(1000) = 
			(SELECT CASE @environment WHEN 'PROD' THEN N'H:\SSIS\Coaching\ConfigFiles\'
									  ...
									  WHEN 'UAT' THEN N'I:\SSIS\Coaching\ConfigFiles\UAT\'
									  ELSE N'G:\SSIS\Coaching\ConfigFiles\' END) -- Assume dev


DECLARE @totalSteps int = 1 -- Only 1 step to call child job when specified file found
DECLARE @stepId int = 0
DECLARE @successAction int = 3 -- go to next step

-------------------------------------------------------------------
-- Object 1:  Step [Call Child Job]
-------------------------------------------------------------------
DECLARE @packageCommand nvarchar(4000) = N'/FILE "' + @packagePath + N'JobTrigger_Coaching.dtsx" /CONFIGFILE "' + @configfilePath + @configfileName +'"  /CHECKPOINTING OFF  /REPORTING E'
DECLARE @stepName nvarchar(100) 
SET @jobId = @mainJobId
SET @stepId = @stepID + 1
SET @stepName =  N'Call Child Job'
