...
DECLARE @jobName nvarchar(100) = N'CoachingSurveyGenerate'
DECLARE @configfileName nvarchar(100) = CONCAT(@environment + N'_', N'eCL_Surveys.dtsConfig')
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@jobName, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Generate Coaching Surveys.', 
...
DECLARE @packagePath nvarchar(1000) = 
			(SELECT CASE @environment WHEN 'PROD' THEN N'\\UVAAPADSQL50CCO\SSIS\Coaching\Packages\'
									  WHEN 'TEST' THEN N'\\UVAADADSQL52CCO\SSIS\Coaching\Packages\eCL_Surveys\' 
									  ELSE N'\\UVAADADSQL50CCO\SSIS\Coaching\Packages\eCL_Surveys\' END) -- Assume dev
...
DECLARE @scriptName nvarchar(100) = N'Notifications_Survey_' + @environment + N'.vbs"' 
...
-------------------------------------------------------------------
-- Object 1:  Step [Generate Coaching Surveys]
-------------------------------------------------------------------
DECLARE @packageCommand nvarchar(4000) = N'/FILE "' + @packagePath + N'eCL_Surveys.dtsx" /CONFIGFILE "' + @configfilePath + @configfileName +'"  /CHECKPOINTING OFF  /REPORTING E'
DECLARE @stepName nvarchar(100) 
SET @jobId = @mainJobId
SET @stepId = @stepID + 1
SET @stepName =  N'Generate surveys'
...
-------------------------------------------------------------------
-- Object 2:  Step [Survey Notifications]
-------------------------------------------------------------------
...
SET @jobId = @mainJobId
SET @stepName =  N' Survey Notifications'
SET @stepId = @stepID + 1
SET @successAction = CASE @stepId WHEN @totalSteps THEN 1 -- quit reporting success
								  ELSE 3 END -- go to next step
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=@stepName, 
