...
namespace eCLDeleteLog
{
    public partial class Search : System.Web.UI.Page
    {
        ...
        /// <returns></returns>
        private string GetViewPageNameBySourceID(string sourceID)
        {
            return IsWarning(sourceID) ? "ViewWarning.aspx" : "View.aspx";
        }

        private bool IsWarning(string sourceID)
        {
            return !string.IsNullOrWhiteSpace(sourceID) && (sourceID == "120" || sourceID == "220");
        }
        ...
        private void ViewDetail(int rowIndex)
        {
            ...
            ViewNameHidden.Value = GetViewPageNameBySourceID(coachingSourceIDHidden.Value);
        ...
        private bool Delete(int rowIndex)
        {
            ...
            string logID = logIDHidden.Value;
            string sourceID = sourceIDHidden.Value;
            ...
            try
            {
                ...
                SqlCommand deleteLogCommand = new SqlCommand(GetDeleteLogSql(logID, sourceID), connection, transaction);
        ...
        /// <returns></returns>
        private string GetDeleteLogReasonSql(string ID, string sourceID)
        {
            if (string.IsNullOrEmpty(ID))
            {
                throw new Exception("Either coachingID or warningID is null or empty.");
            }

            if (string.IsNullOrEmpty(sourceID))
            {
                throw new Exception("sourceID is null or empty for coachingID/warningID[" + ID + "]");
            }
        ...
        /// <param name="coachingID"></param>
        /// <returns></returns>
        private string GetDeleteLogSql(string coachingID, string sourceID)
        {
            if (string.IsNullOrEmpty(ID))
            {
                throw new Exception("Either coachingID or warningID is null or empty.");
