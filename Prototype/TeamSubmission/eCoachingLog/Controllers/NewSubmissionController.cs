    ...
    public class NewSubmissionController : LogBaseController
    {
        ...
        public ActionResult Save(NewSubmissionViewModel vm)
        {
            ...
            if (!ModelState.IsValid)
            {
                ...
                ViewBag.ClientValidateEmployee = vm.ModuleId == 1;// csr 
            ...
            var successfulEmployees = new List<NewSubmissionResult>();
            ...
            try
            {
                ...
                // TODO: remove isDuplicate, since it returns List<SubmissionResults>, duplicate error is one of the returned fields
                submissionResults = this.newSubmissionService.Save(vm, GetUserFromSession());

                //successfulEmployees = submissionResults.Where(x => !String.IsNullOrEmpty(x.LogName)) // x.Error is null or empty
                //        .Select(o =>
                //                new Employee
                //                {
                //                    Id = o.EmployeeId,
                ...
                successfulEmployees = submissionResults.Where(x => !String.IsNullOrEmpty(x.LogName)).ToList();

                //if (successfulEmployees.Count == vm.EmployeeIdList.Count)
                //{
                ...
                //    TempData["ShowSuccessMessage"] = false;
                //    TempData["ShowFailMessage"] = true;
                //    if (successfulEmployees.Count == 0)
                //    {
            ...
            if (successfulEmployees.Count > 0)
            {
                SendEmail(successfulEmployees, vm.ModuleId, sourceId, vm.IsWarning, vm.IsCse, user.EmployeeId);
            }

            return RedirectToAction("Index");
        }

        private void SendEmail(List<NewSubmissionResult> newSubmissionResult, int moduleId, int sourceId, bool? isWarning, bool? isCse, string userId)
        {
            ...
            this.emailService.SendNotification(new MailParameter(newSubmissionResult, moduleId, bIsWarning, bIsCse, iSourceId, template, true, userId));
        }
        ...
        private ActionResult StayOnThisPage(NewSubmissionViewModel vm)
        {
            ...
            // todo: need to count those added on the page (from different site(s))
        ...
        public ActionResult HandleSiteChanged(int siteIdSelected, int programIdSelected)
        {
